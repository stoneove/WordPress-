...
if ( $id || $name ) {
	...
	if ( $post_id ) {
		...
		if ( ! empty( $elements_data ) ) {
			...
			if ( file_exists( $post_css_path ) && 'internal' !== get_option( 'elementor_css_print_method' ) ) {
				$response = wp_remote_get( $upload_url . '/elementor/css/post-' . $post_id . '.css' );
				if ( ! is_wp_error( $response ) && 200 === wp_remote_retrieve_response_code( $response ) ) {
					$shortcodes_custom_css .= wp_remote_retrieve_body( $response );
				}
			} else {
