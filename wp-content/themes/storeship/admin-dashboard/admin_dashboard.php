...
class AF_themes_info{
		...
		function __construct() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s', 'storeship' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s', 'storeship' ), $this->theme_name );
			$this->page_slug     = $this->theme_slug . '-details';
			add_action( 'admin_menu', array( $this, 'storeship_register_info_page' ) );
			add_action( 'admin_enqueue_scripts', array($this,'storeship_register_backend_scripts'));
			add_action( 'init', array($this,'storeship_load_files'));
			add_filter( 'admin_body_class',array($this,'storeship_body_classes') );
		...
		public function storeship_register_info_page() {

			// Add info page.
			add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', $this->page_slug, array( $this, 'storeship_render_page' ), 1 );
		}

		public function storeship_render_page(){?>
			<div id="af-theme-dashboard"></div>
   		<?php }


		function storeship_register_backend_scripts(){
				...
				$changelog = $this->storeship_get_latest_changelog();
				...
				'install_now' => __('Are you sure you want to install this plugin?', 'storeship'),
				'install_btn' => __('Install Now', 'storeship'),
				'activate_btn' => __('Activate', 'storeship'),
				'installed_btn' => __('Activated', 'storeship')
				));
		}

		function storeship_get_latest_changelog(){
			$readme = null;
